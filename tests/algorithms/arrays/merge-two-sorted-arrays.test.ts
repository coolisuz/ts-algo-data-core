...
describe('mergeTwoSortedArrays', () => {
    ...
    test('should handle arrays of significantly different lengths', () => {
        ...
        const expected: number[] = [1,2,4,7,8,10,12,15,19,24,49,50,69,80,99,100,110,176];

        expect(mergeSortedArrays(nums1, num2)).toEqual(expected);
    });

    test('should throw TypeError for non-array inputs', () => {
        expect(() => mergeSortedArrays(null as any, [1,2,3])).toThrow(TypeError);
        expect(() => mergeSortedArrays([1,2,3], undefined as any)).toThrow(TypeError);
    });
});